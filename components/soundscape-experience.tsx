...
function SoundScapePage() {
  ...
  useGSAP(() => {
    ...
    gsap.utils.toArray<HTMLElement>(".practice-section .section-label, .practice-heading h2, .projects-section .section-label, .projects-intro h2, .journal-section .section-label, .journal-copy h2").forEach((element) => {
      gsap.fromTo(element, { y: 28, opacity: 0 }, { y: 0, opacity: 1, duration: 0.9, ease: "power3.out", scrollTrigger: { trigger: element, start: "top 88%", once: true } });
    });
    gsap.utils.toArray<HTMLElement>(".service-row, .project-scene-copy").forEach((element) => {
      gsap.fromTo(element, { y: 22, opacity: 0 }, { y: 0, opacity: 1, duration: 0.75, ease: "power2.out", scrollTrigger: { trigger: element, start: "top 92%", once: true } });
    });
  ...
  return (
    <main ref={rootRef} className="soundscape-shell">
      <header ref={headerRef} className={`site-header${menuOpen ? " is-open" : ""}`}>
        <a className="brand" href="#top" aria-label="SoundScape, volver arriba"><span>SOUNDSCAPE</span></a>
        <nav className={menuOpen ? "main-nav open" : "main-nav"} aria-label="Navegación principal"><a href="#archive" onClick={() => setMenuOpen(false)}>{copy.nav.archive}</a><a href="#services" onClick={() => setMenuOpen(false)}>{copy.nav.services}</a><a href="#projects" onClick={() => setMenuOpen(false)}>{copy.nav.projects}</a><a href="#contact" onClick={() => setMenuOpen(false)}>{copy.nav.contact}</a></nav>
      ...
      <footer className="site-footer"><a className="brand" href="#top"><span>SOUNDSCAPE</span></a><span>{language === "es" ? "Diseño sonoro · música · campo" : "Sound design · music · field"}</span><span>© 2026</span></footer>
